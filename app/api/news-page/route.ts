...
export async function GET() {
    ...
    try {
        const [articlesRes, categoriesRes, authorsRes, commentsRes] = await Promise.all([
            fetch(`${process.env.STRAPI_URL}/api/articles?populate[0]=author&populate[1]=category&populate[2]=cover&pagination[page]=1&pagination[pageSize]=4&sort[0]=publishedAt:desc`, { headers }),
            ...
            fetch(`${process.env.STRAPI_URL}/api/comments?sort=createdAt:desc&pagination[limit]=5&populate[profile]=true`, { headers }),
        ]);
